...
function wpmomo_enqueue_scripts(){
	...
	if(intval(get_option('dmeng_google_code_prettify', 0)))
		wp_enqueue_script('google_code_prettify');
...
function dmeng_get_the_thumbnail($size = 'post-thumbnail'){
	...
	if(!in_array($post_thumbnail_on, array(1, 2)))
		return;
...
function dmeng_settings_error($type = 'updated', $message = ''){
	...
	if(empty( $message ))
		$message = $type == 'updated' ? __('设置已保存。', 'momo') : __('保存失败，请重试。，', 'momo');
...
function dmeng_get_avatar_type($user_id){
	...
	if($avatar == 'qq' && dmeng_is_open_qq($id))
		return 'qq';


	if($avatar == 'weibo' && dmeng_is_open_weibo($id))
		return 'weibo';
	...
	if(get_post_type() == 'post'){
		...
		if(is_single()){
			...
			if(apply_filters('dmeng_post_meta_tag_show', TRUE)){
				...
				if($tags)
					$output .= apply_filters('dmeng_post_meta_tag', '<span itemprop="keywords" class="entry-meta-s">' . $tags . '</span>');
...
function wpmomo_post_nav(){
	...
	<div class="col-lg-6 col-md-6 col-sm-12 random"
		 style=" overflow: hidden; white-space: nowrap;border-top: 1px dashed #E0E0E0; padding-bottom:10px">
		<div>上一篇</div><?php if($previous){
			previous_post_link('&laquo; %link', '%title', TRUE);
		}
		else{
			echo "没有了，已经是最旧文章";
		} ?> </div>
	<div class="col-lg-6 col-sm-6 col-sm-12 random"
		 style="float:none;overflow: hidden; white-space: nowrap; border-top: 1px dashed #E0E0E0;padding-bottom:10px">
		<div>下一篇</div><?php if($next){
...
function dmeng_get_redirect_uri(){
	...
	if(isset( $_GET['redirect_uri'] ))
		return urldecode($_GET['redirect_uri']);
...
function dmeng_publish_post($new_status, $old_status, $post){
	...
	if($new_status == $old_status || $new_status != 'publish' || $post->post_type != 'post')
		return;
...
function dmeng_pager($current, $max){
	...
	if($paged > 1)
		$pager .= '<a class="btn btn-default" href="' . add_query_arg('page', $paged - 1) . '">' . __('上一页', 'momo') . '</a>';


	if($paged < $pages)
		$pager .= '<a class="btn btn-default" href="' . add_query_arg('page', $paged + 1) . '">' . __('下一页', 'momo') . '</a>';
...
function dmeng_wp_footer(){
	...
	switch($theme_color){
		...
		case 1:
			$theme_color    = "#5F6AC0";
			$theme_color_bg = "#4854b5";
			break;

		case 2:
			$theme_color    = "#A52A2A";
			$theme_color_bg = "#872020";
			break;

		case 3:
			$theme_color    = "#f00";
			$theme_color_bg = "#dd0202";
			break;

		case 4:
			$theme_color    = "#00be2d";
			$theme_color_bg = "#01a929";
			break;
	...
	echo '<script>
		...
		var theme_color_bg = "'.$theme_color_bg.'"

	</script>
	<script>

		var float_speed = parseInt('.$float_speed.', 10);
	...
	echo '<script>
		...
		var fancybox_on = "'.$fancybox_on.'";
		var fancybox_helper_on = "'.$fancybox_helper_on.'"
	...
	if(!is_user_logged_in()){
		...
		$script[] = sprintf("var dmengFriend = %s", json_encode(array(
			'title' => __('请同时按下 Ctrl + C 复制推广链接', 'momo'),
			'url'   => add_query_arg('fid', get_current_user_id(), dmeng_canonical_url())
		)));
	...
	if(!is_admin() && !is_preview())
		$script[] = sprintf("var dmengTracker = %s", json_encode(dmeng_tracker_param()));


	$script[] = sprintf("var dmengInstant = %s", ( $instantclick ? 1 : 0 ));


	$script[] = sprintf("var dmengTips = %s", json_encode(array(
		'success'  => __('操作成功', 'momo'),
		'error'    => __('操作失败', 'momo'),
		'tryagain' => __('请重试', 'momo')
	)));
...
function dmeng_wp_head(){
	...
	if($head_css)
		echo '<style type="text/css">' . $head_css . '</style>';
...
function dmeng_check_authentication($username){
	...
	if(dmeng_check_blacklist($username)){
		...
		wp_die(sprintf(__('该%s已被系统保留，不能用于注册或登录！', 'momo'), ( is_email($username) ? __('邮箱地址', 'momo') : __('用户名', 'momo') )), '', array(
			'response'  => 503,
			'back_link' => TRUE
		));
...
function dmeng_only_first_cat_callback($value){
	...
	if(( is_home() || is_archive() ) && !is_category() && dmeng_setting('only_first_cat')){
		...
		if(current_filter() == 'dmeng_the_title'){
			...
			if($category)
				$value = '<span class="first-cat"><a href="' . get_category_link($category[0]->term_id) . '">' . $category[0]->cat_name . '<span class="glyphicon glyphicon-chevron-right"></span></a></span>' . $value;
